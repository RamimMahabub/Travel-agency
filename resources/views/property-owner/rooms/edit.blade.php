        ...
        <div class="card card-body">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @foreach(\App\Models\RoomType::getAmenityOptions() as $amenity)
                    <label class="amenity-tag cursor-pointer" x-data="{ on: {{ in_array($amenity, $existingAmenities) ? 'true' : 'false' }} }" :class="{ 'active': on }">
                        <input type="checkbox" name="amenities[]" value="{{ $amenity }}" class="hidden" x-model="on">
                        <i class="fas fa-check text-[10px]"></i> {{ $amenity }}
                    </label>
                @endforeach
            </div>
        </div>
