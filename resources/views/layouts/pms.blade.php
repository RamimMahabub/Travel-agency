        ...
        >
            ...
            <div class="flex items-center gap-3 px-6 h-20 border-b border-white/10 shrink-0">
                ...
                <div x-show="sidebarOpen" x-transition.opacity.duration.300ms class="truncate">
                    ...
                    <span class="text-[10px] text-brand-primary uppercase tracking-widest font-bold">Partner Hub</span>
                </div>
            </div>
            ...
            <nav class="flex-1 py-6 px-3 space-y-1 overflow-y-auto scrollbar-hide">
                ...
                <a href="{{ route('property-owner.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.dashboard') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-chart-line w-5 text-center {{ request()->routeIs('property-owner.dashboard') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    <span x-show="sidebarOpen" class="font-medium text-sm">Dashboard</span>
                </a>

                <a href="{{ route('property-owner.hotels.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.hotels.*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-hotel w-5 text-center {{ request()->routeIs('property-owner.hotels.*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    <span x-show="sidebarOpen" class="font-medium text-sm">Properties</span>
                </a>

                <div x-show="sidebarOpen" class="px-3 text-[10px] font-bold uppercase tracking-wider text-white/40 mb-2 mt-6">Operations</div>

                <a href="{{ route('property-owner.bookings.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.bookings.*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-calendar-check w-5 text-center {{ request()->routeIs('property-owner.bookings.*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    ...
                    @if(isset($pendingBookingsCount) && $pendingBookingsCount > 0)
                        <span x-show="sidebarOpen" class="bg-brand-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $pendingBookingsCount }}</span>
                    @endif
                </a>
                ...
                <div x-show="sidebarOpen" class="px-3 text-[10px] font-bold uppercase tracking-wider text-white/40 mb-2 mt-6">Growth</div>

                <a href="{{ route('property-owner.promotions.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.promotions.*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-tags w-5 text-center {{ request()->routeIs('property-owner.promotions.*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    <span x-show="sidebarOpen" class="font-medium text-sm">Promotions</span>
                </a>

                <a href="{{ route('property-owner.reviews.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.reviews.*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-star w-5 text-center {{ request()->routeIs('property-owner.reviews.*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    <span x-show="sidebarOpen" class="font-medium text-sm">Reviews</span>
                </a>

                <div x-show="sidebarOpen" class="px-3 text-[10px] font-bold uppercase tracking-wider text-white/40 mb-2 mt-6">Management</div>

                <a href="{{ route('property-owner.guests.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.guests.*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-users w-5 text-center {{ request()->routeIs('property-owner.guests.*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
                    <span x-show="sidebarOpen" class="font-medium text-sm">Guests</span>
                </a>

                <a href="{{ route('property-owner.settings') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group {{ request()->routeIs('property-owner.settings*') ? 'bg-brand-primary/10 text-brand-primary' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-cog w-5 text-center {{ request()->routeIs('property-owner.settings*') ? 'text-brand-primary' : 'text-white/50 group-hover:text-white/80' }}"></i>
            ...
            <div class="p-4 border-t border-white/10 shrink-0">
                <div class="flex items-center gap-3" :class="sidebarOpen ? 'px-2' : 'justify-center'">
                    ...
                    <div x-show="sidebarOpen" class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-white truncate">{{ Auth::user()->name ?? 'Property Owner' }}</p>
                        <p class="text-[11px] text-white/50 truncate">ID: {{ Auth::id() }}</p>
                    </div>
